										...
										array(
											'walker' => new class extends Walker_Nav_Menu {
												function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0) {
													...
													if ($item->ID === 5556) { 
														$output .= '<li id="menu-item-' . $item->ID . '" class="primary-header-button ' . implode(' ', $classes) . '"><a href="' . $item->url . '">' . $item->title . '</a>';
													} else {
														$output .= '<li id="menu-item-' . $item->ID . '" class="' . implode(' ', $classes) . '"><a href="' . $item->url . '">' . $item->title . '</a>';
													}
												}
											},
											'theme_location' => get_current_page_menu(),
											'container'      => '',
											'items_wrap'     => '%3$s',
										)
									);
